...
def password_please() -> str:
    ...
    definitely_not_My_password = 'Benissupercool'
    return definitely_not_My_password
...
def string_list_please() -> list:
    ...
    str_list = ["Tomato", "Cucumber", "Pickle"]
    return str_list


def dictionary_please() -> dict:
    """Returns a dictionary, anything you like."""
    myDic = {'Pickles': 'Sour', "Tomato Sauce": "Heinz", "Internet": "Not NBN"}
    return myDic
...
def greet(name="Towering Timmy") -> str:
    ...
    """
    greeting = ("Well hello, {0}".format(name))
...
def set_it_on_fire(input_string="very naughty boy") -> str:
    ...
    """
    fireString = input_string
    fireString = "🔥".join(input_string)
    fireString = ("🔥{0}🔥".format(fireString))

    return fireString.upper()
...
def best_letter_for_pets() -> str:
    ...
    # filtered = []
    # while i != len(the_alphabet):
    #     for j in range(len(pets)):
    #         if z in the_alphabet in pets[j]:
    #             filtered.append(i)
...
def make_filler_text_dictionary() -> Dict:
    ...
    while low != high + 1:
        if low == 3:
            ...
            for i in range(4):
                low = str(low)
                url = ("https://us-central1-waldenpondpress.cloudfunctions.net/give_me_a_word?wordlength={0}".format(low))
                r = requests.get(url)
...
def random_filler_text(number_of_words=200) -> str:
    ...
    for i in range(number_of_words):
        randomNum = random.randint(3, 7)
        randomNum2 = random.randint(0, 3)
        words.append(my_dict[str(randomNum)][randomNum2])
